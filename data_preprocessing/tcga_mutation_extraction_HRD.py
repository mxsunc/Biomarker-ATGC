...
tissues = ["BRCA","OV","PRAD","PAAD"]
HRD_sample_table = tcga_sample_table[tcga_sample_table["type"].isin(tissues)] 
HRD_sample_table["type"].value_counts()

# load pathology annotations
tumor_types = pd.read_csv(file_path + 'tumor_types_NCI-T.csv', sep='\t')
tumor_types.fillna('', inplace=True)
HRD_sample_table = pd.merge(HRD_sample_table, tumor_types[['type', 'histological_type', 'NCI-T Label', 'NCI-T Code']], how='left', on=['type', 'histological_type'])
...
labels = pd.read_csv(file_path+'/HRDfolds1.csv')
labels = labels.rename(columns={"Patient ID": "bcr_patient_barcode"})
labels = labels[labels['bcr_patient_barcode'].notna()]
tcga_maf = tcga_maf.merge(labels, on="bcr_patient_barcode", how="left")
HRD_maf = tcga_maf[tcga_maf["tissue"].isin(tissues)] 
HRD_maf = HRD_maf[['Hugo_Symbol', 'Center', 'NCBI_Build', 'Chromosome', 'Start_Position', 'End_Position', 'Variant_Classification', 'Variant_Type', 'Reference_Allele', 'Tumor_Seq_Allele2', 'Tumor_Sample_Barcode', 't_ref_count', 't_alt_count', 'Consequence', 'CDS_position', 'STRAND', 'FILTER', 'CONTEXT']]
HRD_maf = HRD_maf.loc[HRD_maf['Chromosome'] != 'MT']
...
HRD_counts = HRD_maf[['Variant_Classification', 'Tumor_Sample_Barcode']].groupby('Tumor_Sample_Barcode').apply(lambda x: pd.Series([len(x), (x['Variant_Classification'].isin(non_syn)).sum()], index=['all_counts', 'non_syn_counts']))
HRD_counts['non_syn_tmb'] = HRD_counts['non_syn_counts'] / 31.85
HRD_counts.reset_index(inplace=True)

# linkage to pancan clinical annotation table via sample barcode
HRD_counts['bcr_patient_barcode'] = HRD_counts['Tumor_Sample_Barcode'].str.extract(r'^([^-]+-[^-]+-[^-]+)-')
HRD_counts['bcr_sample_barcode'] = HRD_counts['Tumor_Sample_Barcode'].str.extract(r'^([^-]+-[^-]+-[^-]+-[^-]+)-')

# join to clinical annotation for data in mc3 only
HRD_sample_table = pd.merge(HRD_sample_table, HRD_counts, how='right', on='bcr_patient_barcode')
HRD_sample_table = HRD_sample_table[HRD_sample_table["type"].isin(tissues)]
df_merged = HRD_sample_table.merge(labels)
...
pickle.dump(df_merged, open(file_path + 'controlled_allmut_combined_HRD_sample_table.pkl', 'wb'))
...
HRD_maf['index'] = HRD_maf.index.values
maf_pr = pr.PyRanges(HRD_maf.loc[:, ['Chromosome', 'Start_Position', 'End_Position', 'index']].rename(columns={'Start_Position': 'Start', 'End_Position': 'End'}))
...
HRD_maf = pd.merge(HRD_maf, result.iloc[:, 3:], how='left', on='index')
...
HRD_maf['Ref'], HRD_maf['Alt'], HRD_maf['five_p'], HRD_maf['three_p'] = variant_features(HRD_maf)

HRD_maf.drop(columns=['index'], inplace=True)

pickle.dump(HRD_maf, open(file_path + 'controlled_allmut_combined_HRD_tcga_maf_table_20.pkl', 'wb'),protocol=4)
pickle.dump(panel_df, open(file_path + 'controlled_allmut_combined_HRD_tcga_panel_table_20.pkl', 'wb'),protocol=4)
# %%
